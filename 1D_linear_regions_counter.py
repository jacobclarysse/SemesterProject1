...
#filepath = "Models_1D/initReLU_1D_D2_V6.h5"
#filepath = "Models_init_1D/ReLU_1D_D1_V6_20.h5"
input_min = 0.0
input_max = 1.0
depth = 4
width = 16
data_dim = 1000 #We take 1000 samples between 0 and 1 and compute the function output
min_data = 0.0
max_data = 1.0
mse_percentage = 0.00
alpha = 0.3
...
print(count_regions(partitions))
###We plot the output of the model as well
x = np.linspace(0,1, 10000)
y_true = act_function(x)
y_pred = model.predict(x)

plt.figure(1)
plt.plot(x, y_true, 'r-')
plt.plot(x, y_pred, 'b-')
layer_1 = in_range(layer_1)
plt.plot(np.array(layer_1), model.predict(np.array(layer_1)), '*')
if depth > 1:
    new_partitions = in_range(new_partitions)
    plt.plot(np.array(new_partitions), model.predict(np.array(new_partitions)), '*')
    if depth > 2:
        layer_3 = in_range(layer_3)
        plt.plot(np.array(layer_3), model.predict(np.array(layer_3)), '*')
        if depth > 3:
            layer_4 = in_range(layer_4)
            plt.plot(np.array(layer_4), model.predict(np.array(layer_4)), '*')
plt.title("Plot of the function and prediction")
if depth == 1:
    plt.legend({"function", "output neural net", "1st layer"})
elif depth == 2:
    plt.legend({"function", "output neural net", "1st layer", "2nd layer"})
elif depth == 3:
    plt.legend({"function", "output neural net", "1st layer", "2nd layer", "3th layer"})
elif depth == 4:
    plt.legend({"function", "output neural net", "1st layer", "2nd layer", "3th layer", "4th layer"})
plt.show()